	...
	local success, returns = pcall(function()
		...
		for match in lines_concats:gmatch("%b()") do
			...
			elseif scriptFuncFound then
				...
				if not s then 
					console:write(console:color("[scriptFunction:ivalid_JSON]: JSONDecode failed.", "#ff0000"))
				...
				if typeof(decode) ~= "table" or not decode.arguments or not decode.commands then
					...
					console:write(console:color("[scriptFunction:ivalid_format]: Invalid function data format to run, JSONDecode was successfully, but the data wasn't expected.", "#ff0000"))
				...
				local function run(c)
					...
					if r[1] == false then
						console:write(console:color("[" .. base_funcName .. "] Error origin in function '" .. base_funcName .. "': " .. tostring(r[2] or "unknown"), "#ff0000"))
						return {false, r[2]}
					end
					if mr == true then
